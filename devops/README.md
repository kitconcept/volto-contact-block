...
DEPLOY_ENV=prod
DEPLOY_HOST=volto-contact-block.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for volto-contact-block.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    volto-contact-block.example.com:
      ansible_user: root
      host: volto-contact-block
      hostname: volto-contact-block.example.com
```
...
Deploy the stack defined in `devops/stacks/volto-contact-block.example.com.yml` to the remote server with:
