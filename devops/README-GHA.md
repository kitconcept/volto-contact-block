...
2. Go to your repository at [collective/volto-contact-block](https://github.com/collective/volto-contact-block).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `volto-contact-block.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | volto-contact-block.example.com | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | volto-contact-block.example.com |
...
- [Backend Tests Workflow](https://github.com/collective/volto-contact-block/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/collective/volto-contact-block/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/collective/volto-contact-block-backend`) and Frontend (`ghcr.io/collective/volto-contact-block-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of volto-contact-block.example.com](https://github.com/collective/volto-contact-block/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/volto-contact-block.example.com.yml` stack.
